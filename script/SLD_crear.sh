...
curl -v -u admin:geoserver -XPOST -d@layergroup.xml -H "Content-type: text/xml" \
  http://localhost/geoserver/rest/workspaces/demo/layergroups

#curl -v -u $login -XPOST -d@layergroup.xml -H "Content-type: text/xml" $restapi/workspaces/$workspace/layergroups
